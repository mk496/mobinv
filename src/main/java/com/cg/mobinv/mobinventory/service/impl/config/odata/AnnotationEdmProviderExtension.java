...
import com.cg.mobinv.mobinventory.service.impl.config.odata.util.ClassHelper;
...
public class AnnotationEdmProviderExtension extends EdmProvider {
  ...
  public AnnotationEdmProviderExtension(final Collection<Class<?>> annotatedClasses) throws ODataException {
    ...
    this.annotatedClasses
        .addAll(annotatedClasses.stream().filter(ANNOTATION_HELPER::isEdmAnnotated).collect(Collectors.toList()));
   ...
   */
  public AnnotationEdmProviderExtension(final String packageToScan) throws ODataException {
    ...
    // }
    // });
    this.annotatedClasses = ClassHelper.loadClasses(packageToScan, ANNOTATION_HELPER::isEdmAnnotated);
    ...
    if (container != null) {
      ...
      for (AssociationSet associationSet : associations) {
        if (associationSet.getAssociation().equals(association)) {
          ...
          if (endTwo.getRole().equals(sourceEntitySetRole) && endTwo.getEntitySet().equals(sourceEntitySetName)) {
            return associationSet;
          }
        }
      }
  ...
  static class TypeBuilder {
    ...
    private Property createComplexProperty(final Field field, final String defaultNamespace) {
      ...
      cp.setType(
          new FullQualifiedName(complexEntityNamespace, ANNOTATION_HELPER.extractComplexTypeName(field.getType())));
      return cp;
    }
